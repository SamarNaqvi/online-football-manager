import { Body, Controller, DefaultValuePipe, Get, ParseIntPipe, Patch, Put, Query, UsePipes} from "@nestjs/common";
import { PlayerService } from "./player.service";
import { ZodValidationPipe } from "src/app/pipes/zod-validation-pipe";
import { BuyPlayerSchema, buyPlayerSchema, PlayerQuerySchema, playerQuerySchema, playerUpdateStatusSchema, PlayerUpdateStatusSchema } from "./player.schema";
...
export class PlayerController{
    ...
    async getTransferListPlayers(@Query() query: PlayerQuerySchema){
        return {players:await this.playerService.getPlayersInTransferList(query?.teamName, query.playerName, query.price)}
    }
    ...
    async changePlayerStatus(@Body() updateData: PlayerUpdateStatusSchema){
        ...
        return await this.playerService.changeTransferListStatus(playerId, status, askingPrice)
    }
